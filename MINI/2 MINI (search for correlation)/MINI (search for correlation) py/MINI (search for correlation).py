...
gamma41 = [s.bitvec('gamma41_'+str(i),1) for i in range(128)]
gamma42 = [s.bitvec('gamma42_'+str(i),1) for i in range(128)]
gamma43 = [s.bitvec('gamma43_'+str(i),1) for i in range(128)]
gamma61 = [s.bitvec('gamma61_'+str(i),1) for i in range(128)]
gamma62 = [s.bitvec('gamma62_'+str(i),1) for i in range(128)]
...
M = [[s.bitvec('M_'+str(i)+'_'+str(j),1) for j in range(16)] for i in range(8)]
...
# split LCT of Sboxes
def SboxExpr(inp,out,ch,Ch,cor):
    filename = 'sboxR' + str(4*(cor+1)) + '.txt'
    ...
    for i in range(16):
        ...
        sum = 's.add(s.or_(s.not_('+str(ch)+'['+str(cor)+']['+str(i)+'] == 1), '+str(Ch)+'['+str(i)+'] == '+str(int(100*(math.log2(64/(cor+1)))))+'))'
        exec(sum)
    file.close()

# combination of tables
def Sbox(inp,out,ch,Ch,sum):
    for i in range(8): SboxExpr(inp,out,ch,Ch,i)
    for i in range(16):
        tc = 's.add(s.or_(s.not_(s.and_('+str(ch)+'[0]['+str(i)+'] == 0'
        for j in range(1,8): tc += ', '+str(ch)+'['+str(j)+']['+str(i)+'] == 0'
        tc += ')), '+str(Ch)+'['+str(i)+'] == 0))'
        exec(tc)
    for i in range(16):
        bij = 's.add('+str(ch)+'[0]['+str(i)+']'
        for j in range(1,8): bij += ' | '+str(ch)+'['+str(j)+']['+str(i)+']'
        bij += ' == '+str(inp)+'['+str(8*i)+'] | '+str(out)+'['+str(8*i)+']'
        for j in range(1,8): bij += ' | '+str(inp)+'['+str(8*i+j)+'] | '+str(out)+'['+str(8*i+j)+']'
        bij += ')'
        exec(bij)
...
def cons(result):
    ...
    tempgamma41 = gamma41[0] ^ result['gamma41_0']
    tempgamma42 = gamma42[0] ^ result['gamma42_0']
    tempgamma43 = gamma43[0] ^ result['gamma43_0']
    tempgamma61 = gamma61[0] ^ result['gamma61_0']
    tempgamma62 = gamma62[0] ^ result['gamma62_0']
    for i in range(1,128):
        ...
        tempgamma41 |= gamma41[i] ^ result['gamma41_'+str(i)]
        tempgamma42 |= gamma42[i] ^ result['gamma42_'+str(i)]
        tempgamma43 |= gamma43[i] ^ result['gamma43_'+str(i)]
        tempgamma61 |= gamma61[i] ^ result['gamma61_'+str(i)]
        tempgamma62 |= gamma62[i] ^ result['gamma62_'+str(i)]
    s.add(tempalpha1 | tempalpha2 | tempalpha3 | tempalpha4 | tempgamma1 | tempgamma3 | tempgamma5 | tempgamma7 | tempgamma41 | tempgamma42 | tempgamma43 | tempgamma61 | tempgamma62  == 1)

# objective function
def obj(lim):
    s.add(Total == S1 + S2 + S3 + S4 + S5 + S6 + S7 + S8 + S9 + S10 + S11 + S12 + S13)
    s.add(Total < lim)
...
def ending(result,ff):
    ...
    for i in range(4):
        for j in range(32):
            ...
            gammam41[i] ^= int(result['gamma41_'+str(32*i+j)]) << j
            gammam42[i] ^= int(result['gamma42_'+str(32*i+j)]) << j
            gammam43[i] ^= int(result['gamma43_'+str(32*i+j)]) << j
            gammam61[i] ^= int(result['gamma61_'+str(32*i+j)]) << j
            gammam62[i] ^= int(result['gamma62_'+str(32*i+j)]) << j
    ...
    print('gamma41: %08x, %08x, %08x, %08x' %(gammam41[3], gammam41[2], gammam41[1], gammam41[0]))
    print('gamma42: %08x, %08x, %08x, %08x' %(gammam42[3], gammam42[2], gammam42[1], gammam42[0]))
    print('gamma43: %08x, %08x, %08x, %08x' %(gammam43[3], gammam43[2], gammam43[1], gammam43[0]))
    print('gamma61: %08x, %08x, %08x, %08x' %(gammam61[3], gammam61[2], gammam61[1], gammam61[0]))
    print('gamma62: %08x, %08x, %08x, %08x' %(gammam62[3], gammam62[2], gammam62[1], gammam62[0]))
    ...
    print('gamma41: %08x, %08x, %08x, %08x' %(gammam41[3], gammam41[2], gammam41[1], gammam41[0]),file=ff)
    print('gamma42: %08x, %08x, %08x, %08x' %(gammam42[3], gammam42[2], gammam42[1], gammam42[0]),file=ff)
    print('gamma43: %08x, %08x, %08x, %08x' %(gammam43[3], gammam43[2], gammam43[1], gammam43[0]),file=ff)
    print('gamma61: %08x, %08x, %08x, %08x' %(gammam61[3], gammam61[2], gammam61[1], gammam61[0]),file=ff)
    print('gamma62: %08x, %08x, %08x, %08x' %(gammam62[3], gammam62[2], gammam62[1], gammam62[0]),file=ff)
...
# multiple solving
def ms(times):
    ff = open('lolmin correlation.txt', 'a')
...
Exchange(gamma1,gamma3,v1)                           # Xor of masks
Exchange(gamma5,gamma7,v2)
Exchange(v1,v2,v3)
Exchange(gamma3,v2,v4)
Exchange(alpha3,gamma62,beta1)
Exchange(alpha2,gamma43,beta2)
Exchange(gamma42,gamma61,v5)
AES_P(w1,gamma41)                                # AES_P transformation
AES_P(w2,gamma42)
AES_P(w3,v4)
AES_P(w4,beta2)                                # AES_P transformation
AES_P(w5,gamma61)
AES_P(w6,v2)
AES_P(w7,beta1)                                # AES_P transformation
AES_P(w8,gamma7)
AES_P(w9,alpha4)
AES_P(w10,alpha1)
AES_P(w11,alpha2)
AES_P(w12,alpha3)
AES_P(w13,alpha4)
Sbox('v3','w1','A','SA','S1')            # S-boxes
Sbox('gamma41','w2','B','SB','S2')
Sbox('gamma42','w3','C','SC','S3')
Sbox('alpha1','w4','D','SD','S4')
Sbox('gamma41','w5','E','SE','S5')
Sbox('v5','w6','F','SF','S6')
Sbox('gamma43','w7','G','SG','S7')
Sbox('gamma61','w8','H','SH','S8')
Sbox('gamma62','w9','I','SI','S9')
Sbox('gamma1','w10','J','SJ','S10')
Sbox('gamma3','w11','K','SK','S11')
Sbox('gamma5','w12','L','SL','S12')
Sbox('gamma7','w13','M','SM','S13')
#cond()                                     # condition

obj(12001)                                # objective function
ms(1)                                      # searching for multiple linear trails
